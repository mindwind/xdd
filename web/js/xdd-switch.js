...
var Lantern = {
	...
	navyTime : 10,// navy����ʱ��
	picMoveSpeed : 22,// ͼƬ�ƶ��ٶ�
	timeOut_time : 7000,// ͣ��ʱ��
	info : // 0.ͼƬurl 1.��� 2.t�ӵ�ַ
	[],
	...
	init : function() {
		...
		for (var i = 0; i < Lantern.info.length; i++) {
			...
			picTemp.style.width = "300px";
			picTemp.style.height = "200px";
			...
			divTemp.name = i;
			divTemp.innerHTML = "<strong>" + (i + 1)
					+ "</strong><span id=\"lanternNc" + i
					+ "\" style=\"display:none\">&nbsp;" + Lantern.info[i][1]
					+ "</span>";
			if (i == 0) {
	...
	initNany : function() {
		...
		for (var j = 0; j < Lantern.info.length; j++) {
			if (j != 0)// δѡ
			{
				...
				document.getElementById("LanternN" + j).style.width = offLength
						+ "px";
				if (j == Lantern.info.length - 1) {
					document.getElementById("LanternN" + j).style.width = (Lantern.width
							- onLength - numtemp)
							+ "px";
				} else {
					numtemp += offLength;
		...
		if (isIE())
			document.getElementById("lanternPoint").style.left = (document
					.getElementById("LanternN0").offsetLeft + 10)
					+ "px";
		else
			document.getElementById("lanternPoint").style.left = (document
					.getElementById("LanternN0").offsetLeft
					- document.getElementById("lantern").offsetLeft + 10)
					+ "px";
		document.getElementById("LanternImg0").style.display = "";
		document.getElementById("LanternImg0").style.left = "0px";
		Lantern.otimeOut = setTimeout("Lantern.cycLantern()",
				Lantern.timeOut_time);
	},
	...
	setNavy : function(i) {
		...
		for (var j = 0; j < Lantern.info.length; j++) {
			...
			if (i != j)// δѡ
			{
				...
				document.getElementById("LanternN" + j).style.width = Lantern.navyCtr[j][0]
						+ "px";
				Lantern.navyCtr[j][2] = (Lantern.navyCtr[j][1] - Lantern.navyCtr[j][0])
						/ Lantern.navyTime;
			} else// ��ѡ
			{
//				Lantern.navyCtr[j][1] = onLength - 34;
//				document.getElementById("LanternN" + j).style.width = Lantern.navyCtr[j][0]
//						- 34 <= 0 ? "0px" : (Lantern.navyCtr[j][0] - 34) + "px";
				Lantern.navyCtr[j][1] = onLength;
				document.getElementById("LanternN" + j).style.width = Lantern.navyCtr[j][1];
				Lantern.navyCtr[j][2] = (Lantern.navyCtr[j][1] - Lantern.navyCtr[j][0])
						/ Lantern.navyTime;
		...
		if (Lantern.onChange) {
			...
			Lantern.oInterval = setInterval('Lantern.changeLantern(' + i + ')',
					10);
		}
	},
	...
	moveNavy : function(select) {
		var breaktime = 0;
		for (var i = 0; i < Lantern.info.length; i++) {
			if ((Lantern.navyCtr[i][2] > 0 && document
					.getElementById("LanternN" + i).offsetWidth < Lantern.navyCtr[i][1])
					|| (Lantern.navyCtr[i][2] < 0 && document
							.getElementById("LanternN" + i).offsetWidth > Lantern.navyCtr[i][1])) {
				if (i == select) {
					document.getElementById("LanternN" + i).style.width = (document
							.getElementById("LanternN" + i).offsetWidth
							+ Lantern.navyCtr[i][2] - 34)
							+ "px";
					document.getElementById("LanternN" + i).style.width = "280px";
				} else {
					document.getElementById("LanternN" + i).style.width = (document
							.getElementById("LanternN" + i).offsetWidth + Lantern.navyCtr[i][2])
							+ "px";
				}
				...
				if (i == select) {
					...
					if (isIE())
						document.getElementById("lanternPoint").style.left = (document
								.getElementById("LanternN" + select).offsetLeft + 10)
								+ "px";
					else
						document.getElementById("lanternPoint").style.left = (document
								.getElementById("LanternN" + select).offsetLeft
								- document.getElementById("lantern").offsetLeft + 10)
								+ "px";
	...
	moveImg : function(i) {
		if (document.getElementById("LanternImg" + i).offsetLeft > 0) {
			document.getElementById("LanternImg" + i).style.left = (document
					.getElementById("LanternImg" + i).offsetLeft - Lantern.picMoveSpeed)
					+ "px";
		} else {
			document.getElementById("LanternImg" + i).style.left = "0px";
			document.getElementById("LanternImg" + Lantern.showNum).style.left = Lantern.width
					+ "px";
	...
	flashImg : function(i) {
		...
		Lantern.otimeOut = setTimeout("Lantern.otimeOut=Lantern.cycLantern()",
				Lantern.timeOut_time);
		Lantern.onChange = false;
	},
